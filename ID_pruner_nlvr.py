...
class Pruner(object):
    def __init__(self, model, args, total_step,
                 mask_param_name=['attn.qkv', 'attn.proj', 'mlp.fc1', 'mlp.fc2','attention.self',
                                  'crossattention.self', \
                                  '.attention.output.dense', 'crossattention.output.dense', 'intermediate.dense',
                                  'output.dense','merge_layer' ],
    ...
    def schedule_threshold_comb(self, step: int, length: int):
        ...
        mask_ind = False
        if step <= initial_warmup * warmup_steps:  # 5400
            threshold = initial_threshold
            mask_ind = False
        elif step > (total_step - final_warmup * warmup_steps):  # all-5400*2
    ...
    def mask_with_threshold(self, model, threshold, ID, useID, global_step):
        ...
        for n, p in model.named_parameters():
            if self.whether_mask_para(n):
                ...
                # p.data[is_dict[n] < this_threshold] = 0.0
    ...
    def update_and_pruning(self, args,model, length, global_step, ID, useID):
        ...
        if (global_step % 3000 == 0) or (global_step % length == 0):
            ...
            f.close()
            compute_prune_ratio(args,model,'blip_nlvr')#Supported types: 'blip_coco', 'blip_nlvr', 'clip_flickr'
            # print("ok")
